...
/** Back-compat: simple realized σ from closes (kept for callers/tests) */
export function computeHistSigmaFromCloses(closes, windowDays = 30) {
  ...
  const m =
    rets.reduce((s, x) => s + x, 0) / Math.max(1, rets.length);
  const varSum = rets.reduce((s, x) => s + (x - m) ** 2, 0);
...
export async function fetchIvATM(symbol, days = 30) {
  try {
    ...
    const r = 0;  // risk-free (annual) — left 0 for neutrality here
    const q = 0;  // dividend yield (annual) — 0 unless caller supplies elsewhere
    ...
    if (valid.length >= 2) {
      ...
      // Map back to below/above by expSec
