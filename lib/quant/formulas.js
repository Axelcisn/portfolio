...
export function erf(x) {
  ...
  const t = 1 / (1 + p * x);
  // Abramowitz & Stegun 7.1.26 with a1 term restored
...
export function d1(S0, K, mu_or_r, sigma_or_q, T_or_sigma, maybeT) {
  const { S0: s0, K: k, mu, sigma, T } = _parseDArgs(arguments);
  const v = volSqrtT(sigma, T);
  if (!(_pos(s0) && _pos(k) && v)) return null;
  return (Math.log(s0 / k) + (mu + 0.5 * sigma * sigma) * T) / v;
}

/** d2 = d1 - v (supports d2(d1, sigma, T) and full signatures) */
export function d2(S0, K, mu_or_r, sigma_or_q, T_or_sigma, maybeT) {
  if (arguments.length <= 3) {
    const [d1v, sigma, T] = arguments;
    return _num(d1v) && _pos(sigma) && _pos(T) ? d1v - sigma * Math.sqrt(T) : null;
  }
  const { S0: s0, K: k, mu, sigma, T } = _parseDArgs(arguments);
  const v = volSqrtT(sigma, T);
  if (!(_pos(s0) && _pos(k) && v)) return null;
  return d1(s0, k, mu, sigma, T) - v;
}
...
/** BSM call with carry q and discount r (positional or object) */
export function bsCall(/* (S0,K,r,q,sigma,T) OR {S0,K,T,sigma,r,q} */) {
  const { S0, K, T, sigma, r, q } = _parseBSArgs(arguments);
  if (!(_pos(S0) && _pos(K) && _pos(sigma) && _pos(T))) return null;
  const d_1 = d1(S0, K, r, q, sigma, T);
  const d_2 = d_1 - sigma * Math.sqrt(T);
  const dfS = Math.exp(-q * T), dfK = Math.exp(-r * T);
  return S0 * dfS * Phi(d_1) - K * dfK * Phi(d_2);
}

/** BSM put with carry q and discount r (positional or object) */
export function bsPut(/* (S0,K,r,q,sigma,T) OR {S0,K,T,sigma,r,q} */) {
  const { S0, K, T, sigma, r, q } = _parseBSArgs(arguments);
  if (!(_pos(S0) && _pos(K) && _pos(sigma) && _pos(T))) return null;
  const d_1 = d1(S0, K, r, q, sigma, T);
  const d_2 = d_1 - sigma * Math.sqrt(T);
...
function _bsAlias(type, ...args) {
  return type === "call" ? bsCall(...args) : bsPut(...args);
}
