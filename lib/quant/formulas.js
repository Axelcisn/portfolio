...
export function erf(x) {
  ...
  const t = 1 / (1 + p * x);
  // Abramowitz–Stegun approximation with nested Horner polynomial
...
export function d2(S0, K, r_or_mu, q_or_sigma, sigma_or_T, maybeT) {
  const { S0: S, K: K0, mu, sigma, T } = _parseDArgs(arguments);
  const d = d1(S, K0, mu, sigma, T);
  const v = volSqrtT(sigma, T);
...
function _bsCallObj({ S0, K, T, sigma, r = 0, q = 0 } = {}) {
...
function _bsPutObj({ S0, K, T, sigma, r = 0, q = 0 } = {}) {
  const s0 = Number(S0), k = Number(K), t = Number(T), sig = Number(sigma);
  const rr = Number(r) || 0, qq = Number(q) || 0;
  if (!(_pos(s0) && _pos(k) && _pos(sig) && _pos(t))) return null;
  const d_1 = d1(s0, k, rr, qq, sig, t);
...
/** BSM call with carry q and discount r. Accepts object or positional args */
export function bsCall(...args) {
  if (args.length === 1 && typeof args[0] === "object") {
    return _bsCallObj(args[0]);
  }
  if (args.length >= 6) {
    const [S0, K, r, q, sigma, T] = args;
    return _bsCallObj({ S0, K, T, sigma, r, q });
  }
  return null;
}

/** BSM put with carry q and discount r. Accepts object or positional args */
export function bsPut(...args) {
  if (args.length === 1 && typeof args[0] === "object") {
    return _bsPutObj(args[0]);
  }
  if (args.length >= 6) {
    const [S0, K, r, q, sigma, T] = args;
    return _bsPutObj({ S0, K, T, sigma, r, q });
  }
  return null;
}
...
function _parseLognArgs(argsLike) {
  ...
  if (a.length === 2 && typeof a[1] === "object") {
    const x = a[0];
    const { S0, T, sigma, mu, r = 0, q = 0 } = a[1] ?? {};
    return { x: Number(x), S0: Number(S0), T: Number(T), sigma: Number(sigma), mu: Number(mu ?? (r - q)) };
  }
  if (a.length >= 6) {
