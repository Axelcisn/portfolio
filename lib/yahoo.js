...
async function jfetch(url, opts = {}) {
  const r = await fetch(url, {
    ...
    headers: {
      ...
      // user-agent helps avoid 401/403 from some Yahoo edges
      "User-Agent":
...
export async function yahooQuote(symbol) {
  ...
  try {
    ...
    if (q) {
      ...
      // fall through to Stooq if spot is missing/zero
    }
...
// Some code may import robustQuote; keep it as an alias to yahooQuote.
