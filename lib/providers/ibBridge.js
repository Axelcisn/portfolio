...
/** Symbol search via IB bridge (stock-only). */
export async function ibSearchCompanies(query, { limit = 8, timeoutMs = 4000 } = {}) {
  const q = String(query || "").trim();
  if (!q) return { ok: true, count: 0, data: [] };
  return ibGet('/v1/search/symbols', { params: { q, limit }, timeoutMs });
}
