...
//
// Next step: optionsProvider will use yahooJson instead of plain fetch.
...
function toCookieHeader(setCookieHeader) {
  // set-cookie can contain multiple cookies; keep the important ones.
  // Fallback: send the whole string trimmed to the first segment pairs.
  const raw = String(setCookieHeader || "");
  if (!raw) return null;

  // Extract cookie name=value pairs (up to the first ';')
  const pairs = raw
    .split(/,(?=[^;]+?=)/g) // split multiple Set-Cookie entries
...
async function initSession() {
  try {
    ...
    try {
      ...
      // Crumb can be optional for some endpoints; keep going with cookie only.
...
/** (Optional) Inspect current session status without touching it. */
export function getYahooSessionInfo() {
  return {
    ...
    // Note: we intentionally do NOT expose the real cookie/crumb values.
  };
}
