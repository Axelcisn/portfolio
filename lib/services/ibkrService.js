    ...
    if (response.ok && Array.isArray(response.data)) {
      return response.data
        ...
        .map(item => ({
          ...
          symbol: item.symbol || item.ticker || query.trim().toUpperCase(),
...
export async function getQuote(symbol) {
  ...
  try {
    ...
    const candidates = await searchSymbols(sym, 5);
    if (!candidates || candidates.length === 0) {
      throw new Error(`Symbol ${sym} not found`);
    }

    const fields = '31,84,86,70,71,82,83,87,88,7295,7296';

    for (const cand of candidates) {
      try {
        const infoRes = await ibRequest(`/iserver/secdef/info?conids=${cand.conid}`);
        const info = Array.isArray(infoRes.data) ? infoRes.data[0] : {};

        const snapshot = await ibRequest(
          `/iserver/marketdata/snapshot?conids=${cand.conid}&fields=${fields}`
        );
        if (!snapshot.ok || !Array.isArray(snapshot.data) || snapshot.data.length === 0) {
          continue;
        }
        const data = snapshot.data[0];
        const price = parseNum(data['31']);
        const bid = parseNum(data['84']);
        const ask = parseNum(data['86']);
        ...
        const out = {
          ...
          currency: info?.currency || cand.currency || null,
          price,
          bid,
          ask,
        ...
        const hasBidAsk = Number.isFinite(bid) || Number.isFinite(ask);
        if (hasPrice || hasBidAsk) {
          return out;
        }
      } catch {
        // try next candidate
