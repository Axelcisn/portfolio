...
import * as mockService from './mockIbkrService.js';

// Check if we should use mock data
function shouldUseMock() {
  // Always use mock if explicitly enabled (works in both dev and prod)
  // Check both server-side and client-side environment variables
  return process.env.USE_MOCK_OPTIONS === 'true' || 
         process.env.NEXT_PUBLIC_USE_MOCK === 'true';
}

// Configuration with automatic fallback and connection preference
function getIBKRConfig() {
  // Priority order for connections:
  // 1. Local proxy bridge (if running)
  // 2. IBKR Gateway (preferred for API access, port 4001)
  // 3. Trader Workstation (port 7496)
  // 4. Environment variable override
  
  // Check environment variable first (can override defaults)
  let baseUrl = process.env.IB_PROXY_URL;
  
  // If not set or if it's a cloudflare URL that might fail, use local defaults
  if (!baseUrl || baseUrl.includes('trycloudflare.com') || baseUrl.includes('cloudflare')) {
    // Prefer IBKR Gateway over TWS for API connections
    // Gateway is more stable and designed for API access
    const useGateway = process.env.USE_IBKR_GATEWAY !== 'false';
    const gatewayPort = process.env.IBKR_GATEWAY_PORT || '5000';
    const twsPort = process.env.IBKR_TWS_PORT || '7496';
    
    if (useGateway) {
      // Use Gateway API port (4001) - Gateway doesn't use /v1/api prefix
      baseUrl = `https://localhost:${gatewayPort}`;
    } else {
      // Fallback to TWS - TWS uses /v1/api prefix
      baseUrl = `https://localhost:${twsPort}/v1/api`;
    }
  }
  ...
  return {
    ...
    timeout: process.env.IBKR_TIMEOUT ? parseInt(process.env.IBKR_TIMEOUT) : 10000,
    // Alternative endpoints for fallback
    fallbackUrls: [
      'https://localhost:5000/v1/api', // Client Portal Gateway default
      'https://localhost:5000',        // Client Portal Gateway (no prefix)
      'https://localhost:4001',        // Gateway API (if configured differently)
      'https://localhost:4001/v1/api', // Gateway API (with prefix)
      'https://localhost:7496/v1/api', // TWS API
      'http://localhost:5055',         // Local proxy if running
    ].filter(url => url !== baseUrl)
  };
}

// Low-level HTTPS request handler with self-signed cert support and automatic fallback
async function ibRequest(path, options = {}) {
  const config = getIBKRConfig();
  let lastError = null;
  
  // Try primary URL first, then fallback URLs if needed
  const urlsToTry = [config.baseUrl, ...config.fallbackUrls];
  ...
  try {
    ...
    if (typeof globalThis.fetch === 'function') {
      try {
        ...
        // If fetch throws (network error or native fetch not usable in this env),
        // fall through to the http/https fallback which tests sometimes mock.
      }
      // Fall through to http/https fallback if fetch returned 404 / threw and alternates didn't help
    }
...
export async function searchSymbols(query, limit = 10) {
  ...
  if (shouldUseMock()) {
    return mockService.searchSymbols(query, limit);
  }
  ...
  try {
    ...
    // In production, fallback to mock service on connection errors
    if ((process.env.VERCEL || process.env.NODE_ENV === 'production') && 
        error.message.includes('IBKR connection failed')) {
      return mockService.searchSymbols(query, limit);
    }
...
export async function getQuote(symbol) {
  ...
  // Use mock service if enabled
  if (shouldUseMock()) {
    return mockService.getQuote(sym);
  }

  try {
    // Step 1: Try to resolve candidate conids via searchSymbols (prefer IBKR search results)
    let conid = null;
    let name = null;
    let exchange = null;
    let currency = null;
    let candidates = [];
    try {
      const searchResult = await searchSymbols(sym, 5);
      if (Array.isArray(searchResult) && searchResult.length > 0) {
        candidates = searchResult;
      }
    } catch (err) {
      // Ignore search errors — we'll try a direct snapshot by symbol below
    }

    // Step 2: Try a direct snapshot using the raw symbol first, but be careful:
    // some mocks return search-like arrays; detect that and fall back to candidate flow.
    const fields = '31,84,86,70,71,82,83,87,88,7295,7296'; // last, bid, ask, high, low, close, change%, volume, 52w high/low
    let snapshot = null;
    let data = null;

    // Attempt raw symbol snapshot (may be useful in test mocks or when direct symbol works)
    try {
      const s0 = await ibRequest(`/iserver/marketdata/snapshot?conids=${encodeURIComponent(sym)}&fields=${fields}`);
      if (s0 && s0.ok && s0.data) {
        const payload0 = Array.isArray(s0.data) ? s0.data[0] : s0.data;
        // If payload looks like a search result (has conid), treat it as not a direct snapshot
        if (!(payload0 && (payload0.conid || payload0.conid === 0))) {
          const px0 = parseNum(payload0?.['31'] ?? payload0?.price ?? payload0?.last);
          const bid0 = parseNum(payload0?.['84']);
          const ask0 = parseNum(payload0?.['86']);
          if ((Number.isFinite(px0) && px0 > 0) || (Number.isFinite(bid0) && Number.isFinite(ask0) && bid0 > 0 && ask0 > 0)) {
            snapshot = s0;
            data = payload0;
            currency = currency || payload0?.currency || payload0?.meta?.currency || currency;
          }
        }
      }
    } catch (e) {
      // ignore and continue to candidate flow
    }

  // Step 3: Try snapshots for each candidate in order until one yields market data
    if (candidates.length > 0) {
      for (const cand of candidates) {
        const candConid = cand.conid;
        try {
          const s = await ibRequest(`/iserver/marketdata/snapshot?conids=${candConid}&fields=${fields}`);
          if (!s || !s.ok) continue;
          const payload = Array.isArray(s.data) ? s.data[0] : s.data;
          // accept if we have any price-like field
          const px = parseNum(payload?.['31'] ?? payload?.price ?? payload?.last);
          const bid = parseNum(payload?.['84']);
          const ask = parseNum(payload?.['86']);
          if (Number.isFinite(px) && px > 0) {
            snapshot = s;
            data = payload;
            conid = candConid;
            name = cand.name || cand.companyName || name;
            exchange = cand.exchange || exchange;
            currency = cand.currency || currency || payload?.currency || payload?.meta?.currency || currency;
            break;
          }
    ...
    // If candidates didn't yield data, try a direct snapshot using the raw symbol
    if (!data) {
      const target = encodeURIComponent(sym);
      snapshot = await ibRequest(`/iserver/marketdata/snapshot?conids=${target}&fields=${fields}`);
      if (snapshot && snapshot.ok) {
        data = Array.isArray(snapshot.data) ? snapshot.data[0] : snapshot.data;
      }
    }

    if (!snapshot || !snapshot.ok || !data) {
    ...
    return {
      ...
      exchange,
      currency,
      price: parseNum(data?.['31'] ?? data?.price ?? data?.last), // Last price
      bid: parseNum(data?.['84']),
      ask: parseNum(data?.['86']),
      high: parseNum(data?.['70']),
      low: parseNum(data?.['71']),
      close: parseNum(data?.['82'] ?? data?.close),
...
export async function getHistoricalData(symbol, period = '1y', bar = '1d') {
  ...
  // Use mock service if enabled
  if (shouldUseMock()) {
    return mockService.getHistoricalData(symbol, period, bar);
  }
...
export async function getOptionsChain(symbol, expiry = null) {
  ...
  // Use mock service if enabled
  if (shouldUseMock()) {
    return mockService.getOptionsChain(symbol, expiry);
  }
...
export async function checkConnection() {
  // Use mock service if enabled
  if (shouldUseMock()) {
    return mockService.checkConnection();
  }
  ...
  try {
    ...
    try {
      await ibRequest('/iserver/marketdata/snapshot?conids=265598&fields=31');
      streams.marketData = { connected: true };
    } catch (err) {
      streams.marketData = { connected: false, cause: err.message };
    }

    // Check account data stream
    try {
      await ibRequest('/iserver/accounts');
      streams.accountData = { connected: true };
    } catch (err) {
      streams.accountData = { connected: false, cause: err.message };
    }
    ...
    return {
      ...
      cause: response.data?.fail || null,
      streams
    };
  } catch (error) {
    // In production, if IBKR connection fails, return mock status
    if (process.env.VERCEL || process.env.NODE_ENV === 'production') {
      return mockService.checkConnection();
    }
    ...
    return {
      ...
      cause,
      streams: {
        marketData: { connected: false, cause },
        accountData: { connected: false, cause }
      }
    };
  }
}
...
// Export low-level request for callers that need to stub or call it directly
export { ibRequest };
