...
function getIBKRConfig() {
  // Use IB_PROXY_URL from environment (Cloudflare tunnel in production)
  // No fallback to localhost - must be explicitly configured
  const baseUrl = process.env.IB_PROXY_URL;
  
  if (!baseUrl) {
    throw new Error('IB_PROXY_URL environment variable is required');
  }
