...
/** Black–Scholes helpers (risk-neutral, continuous div yield). */
function nd(x) {
  // CDF of standard normal via Abramowitz–Stegun approximation
  ...
  const phi = Math.exp(-0.5 * L * L) / Math.sqrt(2 * Math.PI);
...
export function bsCall(S, K, r, q, sigma, T) {
  ...
  return S * Math.exp(-q * T) * nd(_d1) - K * Math.exp(-r * T) * nd(_d2);
}
export function bsPut(S, K, r, q, sigma, T) {
  if (T <= 0 || sigma <= 0) return Math.max(0, K - S);
  const _d1 = d1(S, K, r, q, sigma, T);
  const _d2 = _d1 - sigma * Math.sqrt(T);
  return K * Math.exp(-r * T) * nd(-_d2) - S * Math.exp(-q * T) * nd(-_d1);
}
export function vega(S, K, r, q, sigma, T) {
  if (T <= 0 || sigma <= 0) return 0;
  const _d1 = d1(S, K, r, q, sigma, T);
  return S * Math.exp(-q * T) * pdf(_d1) * Math.sqrt(T);
}
...
export function ivFromChainMid(opt, { S, r = 0, q = 0, T }) {
  ...
  const bid = Number(opt.bid), ask = Number(opt.ask), last = Number(opt.lastPrice ?? opt.last ?? opt.price);
  let mid = null;
...
export function constantMaturityATM(chain, { S0, r = 0, q = 0, cmDays = 30, nowMs = Date.now() }) {
  ...
  return {
    ...
    meta: {
      ...
      T1, T2, Tstar,
      days1: below.days, days2: above.days,
      strike1: atm1.strike, strike2: atm2.strike,
    },
  };
}
