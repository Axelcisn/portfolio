    ...
    <section className="card">
      ...
      <div className="grid grid-3">
        <div className="card"><div className="small">Paths</div>
          <input className="field" type="number" min={100} step={100} value={paths} onChange={e => setPaths(parseInt(e.target.value || "0", 10))} />
        </div>
        <div className="card"><div className="small">Horizon (days)</div>
          <input className="field" type="number" min={1} step={1} value={horizon} onChange={e => setHorizon(parseInt(e.target.value || "0", 10))} />
        </div>
      ...
      <div style={{ marginTop: 12 }}>
        <button className="button" disabled={disabled} onClick={() => onRun?.({ paths, horizon, source })}>Run Simulation</button>
      </div>
    </section>
  );
}
