...
export async function POST(req) {
  ...
  const { spot, mu = 0, sigma = 0, Tdays, paths = 20000,
          legs = {}, netPremium = 0, carryPremium = false, riskFree = 0 } = body || {};
  if (!(spot > 0) || !(Tdays > 0)) return NextResponse.json({ error: "spot>0, Tdays>0 required" }, { status: 400 });
  ...
  for (let i = 0; i < paths; i++) {
    ...
    n++; const d = ST - meanST; meanST += d / n; m2 += d * (ST - meanST);
    const payoff = payoffAt(ST, legs) - carry * netPremium; evAbs += payoff; if (payoff > 0) win++;
    if (i < R) reservoir[i] = ST; else { const j = Math.floor(Math.random() * (i + 1)); if (j < R) reservoir[j] = ST; }
