...
function ok(x, status = 200) {
  return NextResponse.json(x, { status, headers: { "Cache-Control": "no-store" } });
}
...
export async function GET(req) {
  try {
    ...
    const r = await fetch(`${BASE}/v1/company?symbol=${encodeURIComponent(symbol)}`, { cache: "no-store" });
