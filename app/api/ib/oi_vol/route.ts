...
export async function GET(req: Request) {
  try {
    ...
    const url = `${BASE}/options/oi_vol2?symbol=${encodeURIComponent(symbol)}&expiry=${encodeURIComponent(expiry)}&right=${encodeURIComponent(right)}&strikes=${encodeURIComponent(strikes)}`;
    ...
    return NextResponse.json({ error: err?.message || "oi_vol proxy failed" }, { status: 500 });
  }
}
