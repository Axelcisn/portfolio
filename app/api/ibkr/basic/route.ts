...
export async function GET(req: NextRequest) {
  ...
  const pick =
    arr.find(it => Array.isArray(it.sections) && it.sections.some((s:any) => s?.secType === 'STK')) || arr[0];
  ...
  if (!conid) {
    return new Response(JSON.stringify({ ok:false, error:'no conid from search' } satisfies BasicOut), { status: 502 });
  }
