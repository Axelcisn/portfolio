...
export async function GET(req) {
  ...
  try {
    // basic quote: spot, currency, beta, 52w, name
    const q = await yahooQuote(symbol);

    // 1‑year history → realised drift and vol
    let driftHist = null;
    let ivHist   = null;
    try {
    ...
    return NextResponse.json({
      ...
      low52:   q.low52,
      beta:    q.beta,
      ivLive,          // decimal, e.g. 0.31
      ivHist,          // realised annualised vol
      driftHist,       // realised annualised drift
      fxToEUR: fx.rate,
      fxSource: fx.via
    });
  } catch (e) {
