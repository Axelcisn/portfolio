...
/** GET /api/market/stats?index=SPX&lookback=5y&basis=annual&currency=USD&nocache=1 */
export async function GET(req) {
  try {
    ...
    const noCache = (searchParams.get("nocache") || "").trim() === "1";
    ...
    const now = Date.now();
    const hit = !noCache && cache.get(key);
    ...
    const muGeom = s.muGeom ?? null;
    const erp = muGeom == null ? null : (muGeom - rf.rAnnual);
    ...
    const payload = {
      ...
      mrp: erp, // null-safe ERP on annual basis
      meta: {
        window,
    ...
    // 5) Cache and return
    if (!noCache) cache.set(key, { data: payload, expiry: now + TTL_MS });
